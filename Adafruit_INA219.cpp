...
{
  if (_i2c->write_short(reg,value)==0xFFFF);
	syslog(LOG_ERR,"Failed to write short : Adafruit_INA219::wireWriteRegister : %s",
	strerror(errno));	
}
...
void Adafruit_INA219::wireReadRegister(uint8_t reg, uint16_t *value)
{
 ...
 if((*value = _i2c->read_short(reg))==0xFFFF)
	syslog(LOG_ERR,"Failed to read short : Adafruit_INA219::wireReadRegister : %s",
	strerror(errno));
