...
#include <syslog.h>
#include <string.h>		/* Syslog functionality */
#include "I2C.h"
...
uint8_t I2C::read_byte(uint8_t address) {
	if (fd != -1) {
		...
		if (write(fd, buff, BUFFER_SIZE) != BUFFER_SIZE) {
			syslog(LOG_ERR,
					"I2C slave 0x%x failed to go to register 0x%x [read_byte():write %s]",
...
//! Read a single short from I2C Bus
/*!
 \param address register address to read from
 */
uint16_t I2C::read_short(uint8_t address) {
	if (fd != -1) {
		selectDevice();
		...
		msleep(1);
			if (read(fd, &(outputBuffer[1]), BUFFER_SIZE) != BUFFER_SIZE) {
				...
				if (read(fd, &(outputBuffer[0]), BUFFER_SIZE) != BUFFER_SIZE) {
					syslog(LOG_ERR,
							"Could not read from I2C slave 0x%x, register 0x%x [read_byte():read %s]",
							_i2caddr, address, strerror(errno));
					return (-1);
				} else {
				output = (outputBuffer[1]<<8) + (outputBuffer[0]);
				return output;
				}
			}
		}
...
uint8_t I2C::write_byte(uint8_t address, uint8_t data) {
	if (fd != -1) {
		...
		if (write(fd, buff, sizeof(buff)) != 2) {
			syslog(LOG_ERR,
					"Failed to write to I2C Slave 0x%x @ register 0x%x [write_byte():write %s]",
					_i2caddr, address, strerror(errno));
			return (-1);
		} else {
		/*
		...
		syslog(LOG_ERR, "Device File not available. Aborting write");
		return (-1);
	}
	return 0;
}
//! Write a single short to an I2C Device
/*!
 \param address register address to write to
 \param data 8 bit data to write
 */
uint8_t I2C::write_short(uint8_t address, uint16_t data) {
	if (fd != -1) {
		selectDevice();
		...
		buff[1] = (data >> 8);
		buff[2] = (data & 0xFF);
		if (write(fd, buff, sizeof(buff)) != 3) {
			syslog(LOG_ERR,
					"Failed to write to I2C Slave 0x%x @ register 0x%x [write_short():write %s]",
					_i2caddr, address, strerror(errno));
			return (-1);
		} else {
		/*
...
void I2C::openfd() {
	if ((fd = open(busfile, O_RDWR)) < 0) {
		syslog(LOG_ERR, "Couldn't open I2C Bus %d [openfd():open %s]", _i2cbus,
				strerror(errno));
	}
}


void I2C::selectDevice()
{
	if (ioctl(fd, I2C_SLAVE, _i2caddr) < 0) {
		syslog(LOG_ERR, "I2C slave %d failed [openfd():ioctl %s]", _i2caddr,
			strerror(errno));
	}
}